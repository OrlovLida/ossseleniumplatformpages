...
public class ResourceCatalogClient {
    ...
    private static final String GET_SLOT_MODEL_COMPATIBILITIES_URI = "/slotmodel/{id}/compatibilities";
    ...
    public List<CompatibilityDTO> getSlotModelCompatibilities(Long slotModelId) {
        CompatibilityDTO[] compatibilities = env.getResourceCatalogSpecification()
                .given()
                .when()
                .get(GET_SLOT_MODEL_COMPATIBILITIES_URI, slotModelId)
                .then()
                .log().body()
                .assertThat().statusCode(Response.Status.OK.getStatusCode())
                .extract().as(CompatibilityDTO[].class);
        return Arrays.asList(compatibilities);
    }
