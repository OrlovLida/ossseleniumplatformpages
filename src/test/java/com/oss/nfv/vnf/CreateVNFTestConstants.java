package com.oss.nfv.vnf;

public final class CreateVNFTestConstants {

    public static final String ROOT_NODE_PATH = "VNFD_ROOT-1";
    public static final String VNF_ROOT_IDENTIFIER = "VNFSamsungLGU210vSPGWSELENIUMTEST210";
    public static final String VNF_INSTANTIATION_LEVEL_0_IDENTIFIER = "VNFSamsungLGU210vSPGWSELENIUMTEST210VNFDROOTVSPGWFLAVORSELENIUMTEST0";
    public static final String VNF_INSTANTIATION_LEVEL_0_NAME = "VNFD_ROOT_VSPGW-FLAVOR-SELENIUM-TEST-0";

    public static final String NEW_VNF_NAME = "SELENIUM-TEST";

    private CreateVNFTestConstants(){}
}
